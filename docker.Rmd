...
```{bash}
docker build --file Dockerfile_figlet -t mytest_${USER} .
```
...
```{bash}
docker build --no-cache --file Dockerfile_figlet -t mytestfiglet_${USER} .
```
...
docker run mytestfiglet_${USER}

# run with argument "Docker course"
docker run mytestfiglet_${USER} "Docker course"
```
...
```{bash}
docker build -f Dockerfile_RN -t random_numbers_${USER} .
docker run random_numbers_${USER}
docker run random_numbers_${USER} 10
```
...
1. Copy the 2 fastq files from the [Github repository](https://github.com/biocorecrg/PhD_course_containers_2021/tree/main/testdata) and place them in mounted directory.
...
1. Copy the 2 FASTA files from the [Github repository](https://github.com/biocorecrg/PhD_course_containers_2021/tree/main/testdata/blast) and place them in mounted directory.
...
docker run --detach --name webserver_${USER} --publish 80:80 nginx
...
```{block2, type='rdmnote'}
Docker Hub PRO users have the option to have [automatic builds](https://docs.docker.com/docker-hub/builds/) generated when there is a change in an associated Git repository (Github). Alternatively, you can [use Github Actions](https://github.com/marketplace/actions/build-and-push-docker-images).
```
...
```{bash}
docker run -d -v $HOME/db:/blastdb -p 80:8081 --name myblast_${USER} blastwww_${USER}
```
